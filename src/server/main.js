...
class WebServer {
    ...
    async start() {
        try {
            ...
            this.server.listen(process.env.APP_PORT, process.env.APP_HOST, null, () => console.info(`6.1 server is running and reachable on http://${process.env.APP_HOST}:${process.env.APP_PORT}`));
        } catch (_error) {
